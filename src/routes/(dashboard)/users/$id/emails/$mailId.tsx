...
import { mailQueryOptions } from "#/lib/mails";
import type { GraphRecipient } from "#/schema.ts";
...
export const Route = createFileRoute("/(dashboard)/users/$id/emails/$mailId")({
...
function formatRecipients(recipients: Array<GraphRecipient> | null | undefined): string | null {
  if (!recipients?.length) return null;
  const parts = recipients.map((r) => formatAddress(r)).filter(Boolean);
...
function RouteComponent() {
  const { id, mailId } = Route.useParams();
  const token = useSelector(authStore, (auth) => auth?.accessToken);
  const { data: mail, isPending, isError, error } = useQuery(mailQueryOptions(token, id, mailId));

  const subject = mail?.subject?.trim() || "Untitled";
  ...
  return (
    <section className="animate-in rounded-2xl bg-primary px-6 py-8 shadow-xs ring-1 ring-secondary fade-in slide-in-from-bottom-3 sm:px-10 sm:py-10">
      <header className="mb-6 flex flex-wrap items-start gap-3">
        ...
        <Button
          href={`/users/${encodeURIComponent(id)}/emails`}
          color="secondary"
          size="sm"
      ...
      {isError ? (
        ...
        <>
          <dl className="mb-8">
            <Fact label="From">
              <Value>{formatAddress(mail.from)}</Value>
            </Fact>
            <Fact label="To">
              <Value>{formatRecipients(mail.toRecipients)}</Value>
            </Fact>
            <Fact label="Received">
              <Value>{formatWhen(mail.receivedDateTime ?? mail.sentDateTime)}</Value>
            </Fact>
            <Fact label="Status">
              <Badge color={mail.isRead === false ? "brand" : "gray"} size="sm">
                {mail.isRead === false ? "Unread" : "Read"}
              </Badge>
            </Fact>
            <Fact label="Preview">
              <Value>{mail.bodyPreview}</Value>
            </Fact>
          </dl>
